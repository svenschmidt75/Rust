...
mod tests {
    ...
    fn test_network_cost() {
        ...
        let training_data = vec![TrainingData{input_activations: vec![0.0, 0.0].into(), output_activations: vec![0.0].into()}
                                 , TrainingData{input_activations: vec![1.0, 0.0].into(), output_activations: vec![0.0].into()}
                                 , TrainingData{input_activations: vec![0.0, 1.0].into(), output_activations: vec![0.0].into()}
                                 , TrainingData{input_activations: vec![1.0, 1.0].into(), output_activations: vec![1.0].into()}
        ];
