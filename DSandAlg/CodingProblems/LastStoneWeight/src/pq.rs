...
impl<K, V> MaxHeap<K, V>
where
    K: Eq + PartialOrd + Copy,
    V: Copy,
{
    fn new_from_data(data: &[(K, V)]) -> MaxHeap<K, V> {
    ...
    fn find_internal(&self, node_index: usize, value: &(K, V)) -> Option<usize> {
        ...
        if self.heap[node_index].0 == value.0 {
            ...
            // SS: max heap, so if larger than current node, cannot be in tree
    ...
    fn up_heapify(&mut self, node_index: usize) {
        ...
        let mut parent_node = MaxHeap::<K, V>::get_parent(node_index);
    ...
    fn down_heapify(&mut self, node_index: usize) {
        ...
        if let Some(left_child) = self.get_left_child(node_index) {
            if let Some(right_child) = self.get_right_child(node_index) {
                // SS: swap with larger child, so the largest element ends up at the top
    ...
    fn is_maxheap(&self) -> bool {
        self.is_maxheap_internal(0)
    }

    fn is_maxheap_internal(&self, node_index: usize) -> bool {
        ...
        if let Some(left_child) = self.get_left_child(node_index) {
            ...
            if let Some(right_child) = self.get_right_child(node_index) {
                ...
                return self.is_maxheap_internal(left_child)
                    && self.is_maxheap_internal(right_child);
            };
